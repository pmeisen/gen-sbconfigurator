...
public class TestInstantiationOrder {
    ...
    protected IConfiguration runExecution(final String test) {
        ...
        System.setProperty("testBeans.selector", "net/meisen/general/sbconfigurator/config/order/" + test + ".xml");

        // load the configuration
        final ConfigurationCoreSettings configCore = ConfigurationCoreSettings.loadCoreSettings(
    ...
    public void testParameterizedFactory() {
        ...
        final String msg = COMMANDS.toString();
        assertEquals(msg, 3, COMMANDS.size());
        assertTrue(msg, COMMANDS.contains("SimpleBean.constructor"));
        assertTrue(msg, COMMANDS.contains("AutowiredBean.constructor"));
        assertTrue(msg, COMMANDS.contains("AutowiredBean.create"));
    }
    ...
    public void testNestedWiredFactory() {
        ...
        assertTrue(msg, COMMANDS.contains("SimpleBean.constructor"));
        assertTrue(msg, COMMANDS.contains("AutowiredBean.constructor"));
        assertTrue(msg, COMMANDS.contains("ComplexAutowiredBean.constructor"));
        assertTrue(msg, COMMANDS.contains("ComplexAutowiredBean.create"));
    }
    ...
    public void testNestedBeanInFactory() {
        ...
        assertEquals(2, COMMANDS.stream()
                .filter(s -> s.equals("SimpleBean.constructor"))
                .count());
        assertEquals(2, COMMANDS.stream()
                .filter(s -> s.equals("ComplexAutowiredBean.constructor"))
                .count());
        assertEquals(1, COMMANDS.stream()
                .filter(s -> s.equals("AutowiredBean.constructor"))
                .count());
        assertEquals(1, COMMANDS.stream()
                .filter(s -> s.equals("ComplexAutowiredBean.create"))
                .count());
    }
